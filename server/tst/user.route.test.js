...
describe("test /user routes", () => {
  ...
  beforeAll(async () => {
    ...
    const mongoURI =
      ...
      ":" +
      password +
      "@cluster0.dxhaxm8.mongodb.net/?retryWrites=true&w=majority";
  ...
  test("POST /user/auth/login -> Should return status 400 or 401 and a JSON error object with a message", async () => {
    ...
    response = await request(app).post("/user/auth/login").send({
  ...
  test("POST /user/auth/register -> Should return status 200 and a JSON object like {username: someUsername, password: somePassword}", async () => {
    ...
    response = await request(app).post("/user/auth/register").send({
  ...
  // Test register route where a logged in staff member creates a new account for another person here. This should be unsuccessful.
  test("POST /user/auth/change_password -> Should return status 200 and a JSON object like {success: Boolean}", async () => {
    let response;

    // Successfully change password to password11
    response = await request(app).post("/user/auth/change_password").send({
      username: "testSooraj",
    ...
    // Unsuccessfully change password (using incorrect oldPassword)
    response = await request(app).post("/user/auth/change_password").send({
      username: "testSooraj",
      oldPassword: "password12",
      newPassword: "password0",
    });
    expect(response.statusCode).toBe(401); // status for password mismatch
