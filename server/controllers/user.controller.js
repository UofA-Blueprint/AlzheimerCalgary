...
exports.register = (req, res) => {
  ...
  if (Authenticator.checkUsername(username) && Authenticator.checkRole(role)) {
    OnRegisterTokenVerifier.verify(
      username,
      role,
      token,
      process.env.STAFF_SECRET_KEY,
      res
    ); // make sure only a staff can register accounts for the caregivers and the clients
  } else {
...
exports.login = (req, res) => {
  User.findOne({ username: req.body.username }, (err, user) => {
    if (err) {
      return res.status(500).json({
        message: `Failed to find user. This doesn't mean the user doesn't exist. ${err}.`,
      });
    }
    if (!user || !user.comparePassword(req.body.password)) {
      ...
      try {
        let secretKey;
        if (Authenticator.isStaff(user.role)) {
          secretKey = process.env.STAFF_SECRET_KEY;
        } else if (Authenticator.isCaregiver(user.role)) {
          secretKey = process.env.CAREGIVER_SECRET_KEY;
        } else {
          return res.status(401).json({ success: false, token: "undefined" });
        }
        ...
        return res
          .status(500)
          .json({ message: `Failed to create token. ${err}.` });
      }
    }
  });
};
...
exports.sign_up = (req, res) => {
  ...
  if (
    Authenticator.checkUsername(username) &&
    Authenticator.checkStaffKey(staffKey)
  ) {
    NewUserCreator.createStaff(username, password, res);
  } else {
    res.status(400).json({ message: "Invalid username or staff key." });
  }
};
